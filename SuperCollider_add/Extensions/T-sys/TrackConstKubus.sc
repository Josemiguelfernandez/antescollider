...
TrackConstKubus {
	...
	*new {|gbus, channel, fadein, amp, mix_group ... args|
		^super.newCopyArgs(gbus, channel, fadein, amp, *args).initTestargs(gbus, channel, fadein, amp, mix_group, *args);
	}
	...
	initTestargs {|gbus, channel, fadein, amp, mix_group ... args|
		...
		// outbus.postln;
		// "server_toto".postln;
		outsynth = AudioOutKubus.synth_group(group, bus, fadein, outbus, amp);
	...
	gui {|track_name|
		...
		track = TTracksKubus.new(track_name.asString, nil, group, bus, outsynth);
