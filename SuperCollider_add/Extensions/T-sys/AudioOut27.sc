	...
	*def
	{
		...
		^SynthDef(\audioOut27, {|in, out = 0, outbus, gatee = 1, in_ramp = 0.01, out_ramp = 0.01, amp = 0, updateFreq = 10, pos = 0, width = 2, orientation = 0.5, peakLag = 2, index = -1|
			var sig;
			sig = In.ar(in, 27);
			//outbus = in;
			// sig = sig*; //doneAction: 5 libera todos los nodes mas el grupo
			sig = sig * amp.dbamp.lag(1);
			SendPeakRMS.kr(sig, updateFreq, peakLag, '/master_meter_out', index);
