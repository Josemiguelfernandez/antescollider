T {
	...
	*new {
	...
	initT {
		...
		wind =Window("AntesCollider", Rect(385, 550, 300, 500))
				...
				HLayout(
					HLayout(
						VLayout(
							...
							Button().maxHeight_(22).fixedWidth_(50).font_(Font("Helvetica", 11)).states_([["HOA", Color.black, Color.green]]).action_({ // 8 channels
								tracks.insert(trackscount, (\track_++(trackscount+1)).asSymbol -> TTracksKubus.new(\track_++(trackscount+1), aux));
								trackindex.insert(trackscount, trackscount);
