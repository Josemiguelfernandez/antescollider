	...
	*new {|track, aux, grp, bs, otsyn|
	...
	initTTracks {|track, aux, grp, bs, otsyn| // habia "aux" como arg antes...?
		...
		if (bs.notNil) {
			...
			bus = Bus.audio(Server.default, 43);
		};
		...
		if (otsyn.notNil) {
			...
			outsynth = AudioOutKubus.synth(group, bus, bus.index, 0.01, 0.01, 0, 1);
								...
								HLayout(
									Button().maxHeight_(20).maxWidth_(35).states_([["Eq"], ["Eq",Color.white, Color.red]]).action_({|val|
										...
										if (val.value == 1 && eqgui == 0) {
											...
											eq = ChannelEQ.new(43, group, bus, outsynth); // group target define server
											// "group"++group.postln;
											eqgui = 1;
										};
...
/*											"_____________5".postln;
											...
											ante_txt = TextView(wind, Rect(10, 10, 800, 50)).background_(Color.grey).setStringColor(Color.white);

											ante_txt.string = "crea_track_kubus track"++100.rand++" 0 0 0".scatArgs(*[dico_keys.asArray, dico_antes.atAll(modules).asArray.postcs].flop.flatten(3).drop(1));
	...
	addmodule {|module, xfade, par, multi, otros|
		var temp, addsynth, main, background, sliderspec, multisliderspec, typesynth, multiparalist = List[];
		// module.asClass.def;
		main = module.asClass.main;
		background = module.asClass.background;
		typesynth = module.asClass.synthtype;
	...
	addmodfromsynth {|module, synth|
		...
		var parados, parados2, temp, addsynth, main, background, sliderspec, multisliderspec, sliderskey, multisliderskey, typesynth, xfade, par, multi, otros;
