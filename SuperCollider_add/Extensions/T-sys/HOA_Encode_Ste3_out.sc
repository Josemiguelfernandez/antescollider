HOA_Encode_Ste3_out : Tmodule2 {

	*def
	{
		^SynthDef(\HOA_Encode_Ste3_out, {|in, out = 0, amp = 0, gatee = 1, in_ramp = 0.01, out_ramp = 0.01, x1 = 0, y1 = 0, z1 = 0, x2 = 0, y2 = 0, z2 = 0, xoffset = 0, yoffset = 0, zoffset = 0,  dopon = 0, dopamnt = 0, globTBus, spk_radius = 2.5|
			var sig;
			...
			hoa_encoder1 = HOAEncoder.ar(3, dopplershift1, position1.theta, position1.phi, 0, 1,position1.rho, spk_radius); // amp in dB
			hoa_encoder2 = HOAEncoder.ar(3, dopplershift2, position2.theta, position2.phi, 0, 1,position2.rho, spk_radius); // amp in dB
	...
	*metadata
	{
		^(metadata: (
			synthdefname: "HOA_Encode_Ste3_out",
			type: \fx,
			main: \x,
			sliders: [
