HOA_Encode3_out : Tmodule2 {

	*def
	{
		^SynthDef(\HOA_Encode3_out, {|in, out = 0, outbus, outgain = 0, dist = 3, amp = 0, matrix_ramp = 0.01, gatee = 1, in_ramp = 0.01, out_ramp = 0.01, contr = 1, x = 0, y = 0, z = 0, xoffset = 0, yoffset = 0, zoffset = 0, dopon = 0, dopamnt = 0, el = 0, gbfbus, globTBus, llev = 0, glev = 0, sp = 0, df = 0, spk_radius = 2.5|
			...
			// hoa_encoder = HOAConvert.ar(3, HOAEncoder.ar(3, sig, position.theta, position.phi, 0, 1,position.rho, spk_radius), \ACN_N3D, \ACN_SN3D); // amp in dB
			hoa_encoder = HOAEncoder.ar(3, sig, position.theta, position.phi, 0, 1,position.rho, spk_radius); // amp in dB
