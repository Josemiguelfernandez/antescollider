ATK_TransFocus8In : Tmodule2 {

	*def
	{
		^SynthDef(\aTK_TransFocus8In, {|in = 0, inbus, ingain = 0, out = 0, outbus, outgain = 0, xFade = 1, angle = 1.5, dist = 3, azim = 0, amp = 0, matrix_ramp = 0.01, gate = 1, free = 1|
			...
			foa = FoaTransform.ar(foa, 'focus', angle, (azim+45) * (pi/180)); // +45 para que 0° sea L-R
/*			foa = HPF.ar(foa, 20);
	...
	*metadata
	{
		^(metadata: (
			synthdefname: "aTK_TransFocus8In",
			type: \fx,
			main: \azim,
			sliders: [
