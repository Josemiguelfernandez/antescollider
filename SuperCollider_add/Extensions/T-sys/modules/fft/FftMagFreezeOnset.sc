FftMagFreezeOnset : Tmodule2 {

	*def
	{


	^SynthDef(\fftmagFreezeOnset, {|in = 0, inbus, ingain = 0, out = 0, outbus, outgain = -120, xFade = 1, thresh = 0.5, amp = 0, winsize = 0.1, buf = 73, width = 2, orientation = 0.5, lag = 0.2, matrix_ramp = 0.01, gate = 1, free = 1|
			var sig, envgate, envpause, cross, chain, chain2, onsets, fftsize, buffer, pos;
			/*fftsize = 2048;
			...
			chain = PV_MagFreeze(chain, DelayN.kr(abs(onsets - 1), 0.05, 0.05));
			sig = IFFT(chain);
	...
	*metadata
	{
		^(metadata: (
			synthdefname: "fftmagFreezeOnset",
			type: \fx,
			main: \amp,
			sliders: [
