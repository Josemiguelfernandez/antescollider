TCortaExtXPlay : Tmodule2 {


	*def
	{
		^SynthDef(\TCortaExtXPlay, { |out, in, ingain = 1, outbus, inbus, outgain = -120, t_trig= 0, xFade = 1, gran_dur = 0.5,  loop = 0, rate = 1, pos = 0,  trigDev = 0, trigAmp = 0, thresh = 0.5, buf_time = 1, trig_time = 0.5, amp = 0, matrix_ramp = 0.01, gate = 1, free = 1|

			var sig, envgate, envpause, src, env, trigger, env_trig, buf, sr, cross, chain, srconset;
...
/*			chain = FFT(0, srconset);
			...
			sig = XPlayBuf.ar(1, buf, BufRateScale.kr(buf) * rate, trigger, BufFrames.kr(buf) * pos, loop: loop);
			// sig = sig * env_trig;
