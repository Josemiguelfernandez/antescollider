TCortaOnset : Tmodule2 {


	*def
	{
		^SynthDef(\TCortaOnset, { |out, in, ingain = 1, outbus, inbus, outgain = -120, t_trig= 0, xFade = 1, gran_dur = 0.5,  loop = 0, rate = 1, pos = 0,  trigDev = 0, trigAmp = 0, thresh = 0.5, buf_time = 1, trig_time = 0.5, amp = 0, matrix_ramp = 0.01, gate = 1, free = 1|

			var sig, envgate, envpause, src, env, trigger, env_trig, buf, sr, cross, onsets, chain, srconset;
			...
			env = EnvGen.ar(Env.linen(0.01, gran_dur, 0.01), onsets); // variar el porte de los granos
			RecordBuf.ar(src*env, buf, loop: 0, trigger: onsets);
