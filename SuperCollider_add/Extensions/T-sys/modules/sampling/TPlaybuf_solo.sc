TPlaybuf_solo : Tmodule2 {


	*def
	{
		^SynthDef(\TPlaybuf_solo, { |out, outbus, outgain = -120, buf, rev = 1, envbuf = -1, rate = 1.0, pos = 0, loop = 0, amp = 0, t_trig = 0, matrix_ramp = 0.01, gate = 1, free = 1|
			...
			sig = PlayBuf.ar(1, buf, BufRateScale.kr(buf) * rate * rev, t_trig, BufFrames.kr(buf) * pos, loop: loop, doneAction:14); //libera group+synth este libera synth para performance
