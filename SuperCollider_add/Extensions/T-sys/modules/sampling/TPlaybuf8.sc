TPlaybuf8 : Tmodule2 {
	...
	*def
	{
		^SynthDef(\TPlaybuf8, { |out, outbus, outgain = -120, buf, rev = 1, envbuf = -1, rate = 1.0, pos = 0, loop = 0, amp = 0, t_trig = 0, matrix_ramp = 0.01, gate = 1, free = 1|
			...
			sig = PlayBuf.ar(8, buf, BufRateScale.kr(buf) * rate * rev, t_trig, BufFrames.kr(buf) * pos, loop: loop, doneAction:2); //este libera synth para performance
