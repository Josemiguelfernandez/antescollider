Sample_play_seg : Tmodule2 {


	*def
	{
		^SynthDef(\Sample_play_seg, { |out, outbus, outgain = -120, buf, rev = 1, envbuf = -1, rate = 1.0, pos = 0, loop = 0, amp = 0, t_trig = 0, matrix_ramp = 0.01, gate = 1, free = 1, dursecs = 0, id = 0, fade = 0.01, overlap = 0|

			var sig, env, envgate, envpause, line;

			/*			envgate = EnvGen.kr(Env.asr(matrix_ramp, 1.0, matrix_ramp, \welch ), free, doneAction:2);
			envpause = EnvGen.kr(Env.asr(matrix_ramp, 1.0, matrix_ramp, \welch ), gate, doneAction:1);
			*/
			env = EnvGen.kr(Env([0,1,1,0],[fade,dursecs-(fade*2),fade]),doneAction:2);
			line = Line.kr(0, 1, dursecs-overlap);
			sig = PlayBuf.ar(1, buf, BufRateScale.kr(buf) * rate * rev, t_trig, pos, loop: loop); //este libera synth para performance

			SendReply.kr(Done.kr(line), '/seg_end', 1, id);
