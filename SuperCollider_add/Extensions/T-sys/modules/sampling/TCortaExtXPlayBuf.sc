TCortaExtXPlayBuf : Tmodule2 {


	*def
	{
		^SynthDef(\TCortaExtXPlayBuf, { |out, in, ingain = 1, outbus, inbus, outgain = -120, t_trig= 1 xFade = 1, gran_dur = 0.5,  loop = 1, rate = 1, pos = 0,  trigDev = 0, trigAmp = 0, thresh = 0.5, buf_time = 1, trig_time = 1, amp = 0, matrix_ramp = 0.01, gate = 1, free = 1, buf, loopDur = -1, fadeTime = 0.001|

			var sig, envgate, envpause, src, env, trigger, env_trig, sr, cross, chain, srconset;
...
/*			chain = FFT(0, srconset);
			...
			sig = XPlayBuf.ar(1, buf, BufRateScale.kr(buf) * rate, trigger, BufFrames.kr(buf) * pos, loop: loop, loopDur: loopDur, fadeTime:fadeTime);
			// sig = sig * env_trig;
	...
	*metadata
	{
		^(metadata: (
			synthdefname: "TCortaExtXPlayBuf",
			type: \fx,
			main: \amp,
			sliders: [
