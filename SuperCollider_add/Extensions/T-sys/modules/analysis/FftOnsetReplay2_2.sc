FftOnsetReplay2_2 : Tmodule2 {

	*def
	{


	^SynthDef(\fftOnsetReplay2_2, {|in = 0, inbus, ingain = 0, out = 0, outbus, outgain = 0, xFade = 1, trig = 0, amp = 0, winsize = 0.1, thresh = 1, relaxtime = 1, id = 10, matrix_ramp = 0.01, gate = 1, free = 1|
			...
			// onsets.poll;
			SendReply.kr(onsets, '/onsetdetect2', 1, id);
...
/*			sig = WhiteNoise.ar(EnvGen.kr(Env.perc(0.001, 0.1, 0.2), onsets));
	...
	*metadata
	{
		^(metadata: (
			synthdefname: "fftOnsetReplay2_2",
			type: \fx,
			main: \thresh,
			sliders: [
