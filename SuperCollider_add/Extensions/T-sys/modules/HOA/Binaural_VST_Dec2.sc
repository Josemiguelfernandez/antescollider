Binaural_VST_Dec2 : Tmodule2  {

	*def
	{
		^SynthDef(\Binaural_VST_Dec2, {|in, out, gatee = 1, amp = 0, lag = 0.2, matrix_ramp = 0.05,  in_ramp = 0.01, out_ramp = 0.01, gate = 1, free = 1, xFade = 1, buf = 172, azi = 0, ele = 1, spr = 0, freeroom = 0.5, freedamp = 0.5, freemul = 1,  bypass = 0|
			...
			sig = VSTPlugin.ar(sig, 2, bypass); // load IEM VST plugins
	...
	*metadata
	{
		...
		^(metadata: (
			synthdefname: "Binaural_VST_Dec2",
			type: \fx,
			main: \azi,
			sliders:[
