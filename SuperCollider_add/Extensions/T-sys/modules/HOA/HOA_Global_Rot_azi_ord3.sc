HOA_Global_Rot_azi_ord3 : Tmodule2  {

	*def
	{
		^SynthDef(\HOA_Global_Rot_azi_ord3, {|in, out, gatee = 1, amp = 0, lag = 0.2, matrix_ramp = 0.05,  in_ramp = 0.01, out_ramp = 0.01, gate = 1, free = 1,  az = 0|
			var sig, envgate, envpause;

			sig = In.ar(in, 16);

			sig = HOAAzimuthRotator3.ar( // return the Ugen with the b-format channels
	...
	*metadata
	{
		...
		^(metadata: (
			synthdefname: "HOA_Global_Rot_azi_ord3",
			type: \fx,
			main: \azi,
			sliders:[
