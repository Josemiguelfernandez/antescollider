SynthBasic1_Ambi_7 : Tmodule2 {


	*def
	{
		^SynthDef(\SynthBasic1_Ambi_7, {| radius = 0, azi = 0, elev = 0,  globTBus, mul = 0, freq = 36, fc = 1000, rq = 0.25, d = 10, width = 2, orientation = 0.5, amp=0, sawwidth=0, matrix_ramp = 0.01, gate = 1, free = 1, lag = 0.2, compensation = 1, lo_freq = 150, lo_freq_gain = -60, min_sphere = 0.8|
...
/*			position = Cartesian(y, x.neg, z);
			...
			radius = Select.kr(radius < 0.001, [radius, 0.001]);
			// dis2 = distance;
			...
			hoa_encoder = AmbisonicEncoder.ar(7, sig, 0, radius, azi, elev);
	...
	*metadata
	{
		^(metadata: (
			synthdefname: "SynthBasic1_Ambi_7",
			type: \gen,
			main: \freq,
			sliders:  [
