HOA_Encode4 : Tmodule2 {

	*def
	{
		^SynthDef(\HOA_Encode4, {|in, amp = 0, x = 0, y = 0, z = 0, xoffset = 0, yoffset = 0, zoffset = 0, globTBus, buf, envbuf, pos = 0, loop = 0, t_trig = 0, rate = 1|
...
/*			x = Lag.kr(x, 0.1);
			...
			hoa_encoder = HOAEncoder.ar(4, sig, position.theta, position.phi, 0, 1,position.rho); // amp in dB ne marche pas
