HOA_Play_samp_xyz3 : Tmodule2 {

	*def
	{
		^SynthDef(\HOA_Play_samp_xyz3, {|amp = 0, x = 0, y = 0, z = 0, globTBus, buf, envbuf, pos = 0, loop = 0, t_trig = 0, rate = 1, rev = 1|
			...
			hoa_encoder = HOAEncoder.ar(3, sig, position.theta, position.phi, amp, 1, position.rho); // amp in dB
	...
	*metadata
	{
		^(metadata: (
			synthdefname: "HOA_Play_samp_xyz3",
			type: \gen,
			main: \amp,
			sliders: [
