HOA_Grain_Interp_del_xyz7 : Tmodule2 {

	*def
	{
		^SynthDef(\HOA_Grain_Interp_del_xyz7, {|in, amp = 0, x = 0, y = 0, z = 0, xoffset = 0, yoffset = 0, zoffset = 0, globTBus, buf, envbuf0=0, envbuf1=0, pos = 0, loop = 0, t_trig = 0, rate = 1, env_dur = 1, env_interp = -1, del = 0, max_del_time = 1, wind_steep = 0.0005|
			...
			DetectSilence.ar(sig, time:0.1,  doneAction: Done.freeSelf); // free grain when silence
	...
	*metadata
	{
		^(metadata: (
			synthdefname: "HOA_Grain_Interp_del_xyz7",
			type: \gen,
			main: \amp,
			sliders: [
