HOA_Global_Mirror_ord1 : Tmodule2  {

	*def
	{
		^SynthDef(\HOA_Global_Mirror_ord1, {|in, out, gatee = 1, amp = 0, lag = 0.2, matrix_ramp = 0.05,  in_ramp = 0.01, out_ramp = 0.01, gate = 1, free = 1,  front_back = 0, left_right = 0, up_down = 0|
			var sig, envgate, envpause;

			sig = In.ar(in, 4);

			sig = HOAMirror1.ar( // return the Ugen with the b-format channels
	...
	*metadata
	{
		...
		^(metadata: (
			synthdefname: "HOA_Global_Mirror_ord1",
			type: \fx,
			main: \azi,
			sliders:[
