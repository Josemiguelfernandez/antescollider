HOA_Global_Lev50_Dec1 : Tmodule2  {

	*def
	{
		^SynthDef(\HOA_Global_Lev50_Dec1, {|in, out, gatee = 1, amp = 0, lag = 0.2, matrix_ramp = 0.05,  in_ramp = 0.01, out_ramp = 0.01, gate = 1, free = 1,  speakers_radius = 1.07, hoa_buf_init|
			var sig, envgate, envpause, cross, convsig, b2a, a2b, hoa;

			sig = In.ar(in, 4);

			// sig = HOADecLebedev50.ar(1, sig, hrir_Filters:1);
			hoa = HOADecLebedev501.ar(
	...
	*metadata
	{
		...
		^(metadata: (
			synthdefname: "HOA_Global_Lev50_Dec1",
			type: \fx,
			main: \azi,
			sliders:[
