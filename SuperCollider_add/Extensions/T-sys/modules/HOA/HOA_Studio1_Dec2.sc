HOA_Studio1_Dec2 : Tmodule2  {

	*def
	{
		^SynthDef(\HOA_Studio1_Dec2, {|in, out, gatee = 1, amp = 0, lag = 0.2, matrix_ramp = 0.05,  in_ramp = 0.01, out_ramp = 0.01, gate = 1, free = 1, xFade = 1, buf = 172, azi = 0, ele = 1, spr = 0, freeroom = 0.5, freedamp = 0.5, freemul = 1|
			var sig, envgate, envpause, cross, convsig, b2a, a2b;

			sig = In.ar(in, 9);
			sig = sig * amp.dbamp.lag(1);

			sig = HOADecStudio1.ar(2, sig, 0);
