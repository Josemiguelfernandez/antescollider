SynthBasic1_HOA_2 : Tmodule2 {


	*def
	{
	^SynthDef(\SynthBasic1_HOA_2, {| x = 0, y = 0, z = 0,  globTBus, mul = 0, freq = 36, fc = 1000, rq = 0.25, d = 10, width = 2, orientation = 0.5, amp=0, sawwidth=0, matrix_ramp = 0.01, gate = 1, free = 1, lag = 0.2|
		var sig, envgate, envpause, position, hoa_encoder;
			...
			hoa_encoder = HOAEncoder.ar(2, sig, position.theta, position.phi, 0, 1,position.rho); // amp in dB
	...
	*metadata
	{
		^(metadata: (
			synthdefname: "SynthBasic1_HOA_2",
			type: \gen,
			main: \freq,
			sliders:  [
