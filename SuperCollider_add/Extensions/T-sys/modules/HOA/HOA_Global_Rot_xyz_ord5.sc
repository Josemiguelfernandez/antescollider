HOA_Global_InPhase4 : Tmodule2  {

	*def
	{
		^SynthDef(\HOA_Global_InPhase4, {|in, out, gatee = 1, amp = 0, lag = 0.2, matrix_ramp = 0.05,  in_ramp = 0.01, out_ramp = 0.01, gate = 1, free = 1|
			var sig, envgate, envpause;

			sig = In.ar(in, 25);

			sig = HOALibInPhase3D4.ar( // return the Ugen with the b-format channels
	...
	*metadata
	{
		...
		^(metadata: (
			synthdefname: "HOA_Global_InPhase5",
			type: \fx,
			main: \azi,
			sliders:[
