HOA_Global_Rot_xyz_ord2 : Tmodule2  {

	*def
	{
		^SynthDef(\HOA_Global_Rot_xyz_ord2, {|in, out, gatee = 1, amp = 0, lag = 0.2, matrix_ramp = 0.05,  in_ramp = 0.01, out_ramp = 0.01, gate = 1, free = 1,  pitch = 0, roll = 0, yaw = 0|
			var sig, envgate, envpause;

			sig = In.ar(in, 9);

			sig = HOARotator2.ar( // return the Ugen with the b-format channels
	...
	*metadata
	{
		...
		^(metadata: (
			synthdefname: "HOA_Global_Rot_xyz_ord2",
			type: \fx,
			main: \azi,
			sliders:[
