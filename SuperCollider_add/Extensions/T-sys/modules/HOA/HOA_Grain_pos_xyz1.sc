HOA_Grain_pos_xyz1 : Tmodule2 {
// posittione of buffer in seconds for concatanaive synth
	*def
	{
		^SynthDef(\HOA_Grain_pos_xyz1, {|amp = 0, x = 0, y = 0, z = 0, xoffset = 0, yoffset = 0, zoffset = 0, globTBus, buf, envbuf, pos = 0, loop = 0, t_trig = 0, rate = 1, env_dur = 1|
			...
			hoa_encoder = HOAEncoder.ar(1, sig*env, position.theta, position.phi, 0, 1,position.rho); // amp in dB ne marche pas
	...
	*metadata
	{
		^(metadata: (
			synthdefname: "HOA_Grain_pos_xyz1",
			type: \gen,
			main: \amp,
			sliders: [
