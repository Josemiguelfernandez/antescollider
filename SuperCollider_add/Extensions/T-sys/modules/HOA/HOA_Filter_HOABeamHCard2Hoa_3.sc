HOA_Filter_HOABeamHCard2Hoa3 : Tmodule2  {

	*def
	{
		^SynthDef(\HOA_Filter_HOABeamHCard2Hoa3, {|in, out, gatee = 1, amp = 0, lag = 0.2, matrix_ramp = 0.05, gate = 1, free = 1, xFade = 1, buf = 172, spr = 0, freeroom = 0.5, freedamp = 0.5, freemul = 1, az = 0, ele = 0, cardOrder = 1|
			var sig, envgate, envpause, cross, convsig, b2a, a2b;


			sig = In.ar(in, 16);

			sig = HOABeamHCard2Hoa.ar(3, sig, Lag.kr(-1*az, 0.1), Lag.kr(ele, 0.1), cardOrder);
	...
	*metadata
	{
		...
		^(metadata: (
			synthdefname: "HOA_Filter_HOABeamHCard2Hoa3",
			type: \fx,
			main: \azi,
			sliders:[
