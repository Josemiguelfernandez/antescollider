TestSynth2_2 : Tmodule2 {


	*def
	{
	^SynthDef(\TestSynth2_2, {|out, outbus, outgain = 0, note = 36,  note2= 45, fc = 1000, rq = 0.25, bal = 0, amp=0, width=0, matrix_ramp = 0.01, gate = 1, free = 1|
		var sig, sig2, envgate, envpause;
			...
			sig2 = Mix.fill(4, {
				VarSaw.ar((note2 + {0.1.rand2}).midicps, 0, width, 0.02)
			});
			sig2 = RLPF.ar(sig2, fc, rq).softclip;
			sig2 = RLPF.ar(sig2, fc, rq, amp.dbamp).softclip;

			//x = Balance2.ar(x[0], x[1], bal);
			//x = Pan2.ar(x, -1);
			sig = [sig, sig2] * envgate * envpause;
