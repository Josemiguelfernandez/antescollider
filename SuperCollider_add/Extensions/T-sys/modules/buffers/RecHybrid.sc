RecHybrid : Tmodule2 {
	...
	*def
	{
		^SynthDef(\RecHybrid, { |in = 0, buf, buf_fft, rec_dur = 1, loop = 0, fftSize=4096, hop= 0.25 window=1, doneAction = 2, fade_in = 0.01,  fade_out = 0.01, gate = 1, in_gain = 0|
			...
			sig = Limiter.ar(In.ar(in*in_gain.dbamp.lag, 1), -3.dbamp);
			// sig.poll;
			bufnum = LocalBuf.new(fftSize, 1);
			chain = FFT(bufnum, sig, hop, window); // sig*env
			chain = PV_RecordBuf(chain, buf_fft, 0, 1, loop, hop, window);
			RecordBuf.ar(sig, buf.clear, doneAction: doneAction, loop: loop);
