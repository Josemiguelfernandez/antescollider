TBufRdEnvInterp : Tmodule2 {


	*def
	{
		^SynthDef(\TBufRdEnvInterp, { |out = 0, buf, loop = 1, rate=1, envbuf0, envbuf1, impulse_rate = 1, pos=0, interp = -1|
			...
			phasor = Phasor.ar(trig, impulse_rate/sr, 0, 1);
			env = BufRd.ar(1, [envbuf0, envbuf1], phasor*BufFrames.kr(envbuf0));

			env = LinXFade2.ar(env[0], env[1], interp); // -1 to 1
...
/*			phasor_env = Phasor.ar(trig, ((SampleRate.ir/BufFrames.kr(envbuf)).reciprocal*env_dur.reciprocal)*rate, 0.0, BufFrames.kr(envbuf);
	...
	// a =Synth(\TBufRdEnvInterp, [out: 0, impulse_rate:1])
