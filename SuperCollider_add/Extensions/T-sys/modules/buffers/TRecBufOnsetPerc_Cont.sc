TRecBufOnsetPerc_Cont : Tmodule2 {


	*def
	{
		^SynthDef(\recbuffonsetperccont, { |in = 0, buf = 185, rec_dur = 2, threshes = 1, trigdel = 1, amp = 0, antirebond = 0|
...
/*			(
			...
			env = EnvGen.ar(Env.perc(0, rec_dur, 1, 0.5), trig, doneAction: 0); // variar el porte de los granos
	...
	*metadata
	{
		...
		^(metadata: (
			synthdefname: "recbuffonsetperccont",
			type: \gen,
			main: \amp,
			sliders: [
