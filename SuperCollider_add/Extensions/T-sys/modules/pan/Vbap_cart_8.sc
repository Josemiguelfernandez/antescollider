Vbap_cart_8 : Tmodule2  {

	*def
	{

		^SynthDef(\Vbap_cart_8, {|in, out = 0, outbus, outgain = -120, gatee = 1, in_ramp = 0.01, out_ramp = 0.01, amp = 0, x = 0, y = 0, z = 0, spr = 0, width = 60, vbapBuf, dopon = 0, dopamnt = 0, lag = 0, matrix_ramp = 0, gate = 1, free = 1, xFade = 1|
			var sig, envgate, envpause, cross, minRadRoot, radRoot, radio, position, azi, distance;
			...
			azi = position.theta.circleRamp;
			minRadRoot = 0.076923076923077; // from Mosca spatializer
			radio = distance.linlin(0, 10, 0, 1);
			...
			sig = VBAP.ar(8, sig*(1 - radRoot), vbapBuf, azi, position.phi, spr);
