	...
	*def
	{
		...
		^SynthDef(\TVbap16Zig, {|in, out = 0, outbus, outgain = -120, gatee = 1, amp = 0, lag = 0.2, matrix_ramp = 0.05, gate = 1, free = 1, xFade = 1, buf = 174, azig = 0.5, elemod = 3, spr = 0|
			...
			sig = VBAP.ar(16, sig, buf, LFSaw.kr(azig, 0).range(-180, 180) * -1, SinOsc.kr(elemod, 0).range(0, 14.97), spr);
