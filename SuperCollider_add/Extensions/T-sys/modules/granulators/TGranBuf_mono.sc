TGranBuf_mono : Tmodule2 {


	*def
	{
		^SynthDef(\TGranBuf_mono, { |in = 0, out = 22, outbus = -2, trigRate = 1, trigRateDev = 0, buf = 1, envbuf = -1, t_trig = 0, rate = 1.0, rateDev = 0, startPos= 0, centerDev = 0, dur = 0.25, durDev = 0, pan = 0, panDev = 0, amp=0, ampDev = 0, interpol = 2, matrix_ramp = 0.01, gate = 1, free = 1, xFade = 1|

			var sig, impulso, posDevia, pos, trigRate2, envgate, envpause, cross, granout;
			...
			granout = GrainBuf.ar(1, trigRate2, dur, buf, rate, startPos, interpol, pan , envbuf);
			//sig = sig * EnvGen.kr(Env([0, 1, 0], [1, 1], \sin, 1), gate, doneAction: 2);
			granout = granout * envgate * envpause;

			// ReplaceOut.ar(out, sig*amp.dbamp.lag);
