TGranBuf_RecIn4 : Tmodule2 {


	*def
	{
		^SynthDef(\TGranBuf_RecIn4, { |in = 0, out = 22, outbus = -2, trigRate = 1, trigRateDev = 0, buf = 1, envbuf = -1, t_trig = 1, rate = 1.0, rateDev = 0, startPos= 0, centerDev = 0, dur = 0.25, durDev = 0, pan = 0, panDev = 0, amp=0, ampDev = 0, interpol = 2, matrix_ramp = 0.01, gate = 1, free = 1|

			var sig, sig_out, impulso, posDevia, pos, trigRate2, envgate, envpause;
			...
			sig_out = GrainBuf.ar(4, trigRate2, dur, buf, rate, startPos, interpol, pan , envbuf);
			//sig = sig * EnvGen.kr(Env([0, 1, 0], [1, 1], \sin, 1), gate, doneAction: 2);
			sig_out = sig_out * envgate * envpause;

			ReplaceOut.ar(out, sig_out*amp.dbamp.lag);
