TGranFreeze_Onset : Tmodule2 {

	*def
	{


		^SynthDef(\TGranFreeze_Onset, {|in = 0, inbus, ingain = 0, out = 0, outbus, outgain = -120, xFade = 1, buf = 1, amp = 0, trate =  60, rate = 1, rateDev = 0, tratedev = 0, pan = 0, panDev = 0, dur = 0.1, durDev = 0.01, matrix_ramp = 0.01, startPos = 0.1, gate = 1, free = 1, thresh = 0.5, reset = 0|
			var sig, envgate, envpause, cross, clk, pos,  chain, onsets, trig; //dur,
			...
			sig = TGrains.ar(1, clk, buf, rate, pos, dur, pan, 0.4);
	...
	*metadata
	{
		^(metadata: (
			synthdefname: "TGranFreeze_Onset",
			type: \fx,
			main: \amp,
			sliders: [
