HOA_Encode_1ch7 : Tmodule2 {

	*def
	{
		^SynthDef(\HOA_Encode_1ch7, {|in, out = 0, amp = 0, gatee = 1, in_ramp = 0.01, out_ramp = 0.01|
			var sig;
			var gain, coords, n=1, radius, az, ele;
			...
			sig = AmbisonicEncoder.ar(7, sig, gain, coords);
			sig = sig*EnvGen.ar(Env.asr(in_ramp, 1, out_ramp, \welch ), gatee);
