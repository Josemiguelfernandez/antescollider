HOA_Encode2_2ch7_out : Tmodule2 {

	*def
	{
		^SynthDef(\HOA_Encode2_2ch7_out, {|in, out = 0, amp = 0, gatee = 1, in_ramp = 0.01, out_ramp = 0.01, xoffset = 0, yoffset = 0, zoffset = 0,  dopon = 1, dopamnt = 0, globTBus|
			var sig;
			var gain, coords, doppler, n=2;

			gain = \gain.kr(([0.0] ! n).flat);
			coords = \coords.kr(([1.0, 0.0, 0.0] ! n).flat);
			doppler = \doppler.kr(([1] ! n).flat);
			sig = In.ar(in, n);
			sig = AmbisonicEncoderV2_in2.ar(7, sig, gain, coords, doppler);
	...
	*metadata
	{
		^(metadata: (
			synthdefname: "HOA_Encode2_2ch7_out",
			type: \fx,
			main: \x,
			sliders: [
