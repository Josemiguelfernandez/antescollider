	...
	*new {|track, aux, grp, bs, otsyn|
	...
	initTTracks {|track, aux, grp, bs, otsyn| // habia "aux" como arg antes...?
		...
		if (otsyn.notNil) {
			...
			outsynth = AudioOut16.synth(group, bus, bus.index, 0.01, 0.01, 0, 1);
								...
								HLayout(
									Button().maxHeight_(20).maxWidth_(35).states_([["Eq"], ["Eq",Color.white, Color.red]]).action_({|val|
										...
										if (val.value == 1 && eqgui == 0) {
											...
											eq = ChannelEQ.new(16, group, bus, outsynth); // group target define server
											// "group"++group.postln;
											eqgui = 1;
										};
...
/*											"_____________5".postln;
											...
											ante_txt.string = "crea_track16 track"++100.rand++" 0 0 0".scatArgs(*[dico_keys.asArray, dico_antes.atAll(modules).asArray.postcs].flop.flatten(3).drop(1));
