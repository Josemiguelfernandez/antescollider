...
TTracks8 {
	...
	*new {|track, aux, grp, bs, otsyn|
	...
	initTTracks {|track, aux, grp, bs, otsyn| // habia "aux" como arg antes...?
		...
		win = Window.new(ttrack, Rect(385,0,310,450)).background_(Color.new255(120, 120, 120)).alpha_(0.9); // false
		...
		if (otsyn.notNil) {
			...
			outsynth = AudioOut8.synth(group, bus, bus.index, 0.01, 0.01, 0, 1);
		...
		win.layout = VLayout(
			...
			View().maxHeight_(80).fixedWidth_(308)
					...
					View().maxWidth_(310).layout_(
						HLayout(
							...
							VLayout(levelmeter = LevelIndicator().maxHeight_(14).fixedWidth_(187).numTicks_(37).numMajorTicks_(5).warning_(0.88).critical_(0.98).drawsPeak_(true),
								...
								nil,
								HLayout(
									Button().maxHeight_(20).maxWidth_(35).states_([["Eq"], ["Eq",Color.white, Color.red]]).action_({|val|

										if (val.value == 1 && eqgui == 0) {

											eq = ChannelEQ.new(8, group, bus, outsynth); // group target define server
											// "group"++group.postln;
											eqgui = 1;
										};
										if (val.value == 1 && eqgui == 1) {eq.visible};
										if(val.value == 0) {eq.unlinkGUI};
										// if(val.value == 0) {"eq_off".postln; eqgui = 0; eq.unlinkGUI}

									}),
									Button().maxHeight_(20).maxWidth_(35).states_([["☯"]]).action_({}),
									Button().maxHeight_(20).maxWidth_(50).states_([["antes"]]).action_({
										var lista_antes, modules,lista2antes = List[], dico_antes, dico_keys, wind, ante_txt;
										// val.value.postln;
										/*"_____________0".postln;
										store.postcs;
										"_____________1".postln;*/
										modules = store.collect(_.first);
										lista_antes = store;
										// lista_antes.postcs;
										// "_____________2".postln;
										dico_antes = Dictionary.new;
										modules.collect({|mod|
											dico_antes.put(mod, List[]);
											// modules.postcs;
										});
										// "_____________22".postln;
										// dico_antes.postcs;
										// "_____________23".postln;
										// dico_antes.order.postcs;
										// "_____________3".postln;
										// dico_antes.order.postln;
										// "_____________4".postln;
										// modules.postln;

										modules.collect({|mo, a|
											mo.asClass.paramslider.collect({|sp, i|
												var slider_name, slider_sp;
												slider_name = sp.key;
												slider_sp = sp.value;
												synthe[a].get(slider_name, {|val|
													dico_antes[mo].add([slider_name, val]);
													// dico_antes[mo].postln;
												});
											});
										});
										modules.collect({|mo, a|
											mo.asClass.parammultislider.collect({|sp, i|
												var multislider_name, multislider_sp, multi_size;
												multislider_name = sp.key;
												multislider_sp = sp.value[0];
												multi_size = sp.value[1];
												synthe[a].getn(multislider_name, multi_size, {|val|
													dico_antes[mo].add([multislider_name, "("++val++")"]);
													// dico_antes[mo].postln;
												});
											});
										});
										...
										// lista_antes.postln;
										// lista2antes.postln;
										{0.01.wait;{ //espera respuesta de parametros del servidor
											// dico_keys = dico_antes.order;
											dico_keys = [("#->".asSymbol)!modules.size, modules].flop; //.drop(1)
											/*"___________________".postln;
											dico_keys.postln;
											"___________________".postln;*/
											/*											"_____________5".postln;
											dico_keys.postcs;
											"_____________6".postln;
											// d.atAll(d.order);
											dico_antes.atAll(modules).postcs;
											"_____________7".postln;*/
											// values.asArray.postcs;
											// "".scatArgs(*[dico_keys.asArray, dico_antes.values.asArray].flop.flatten(3).drop(1)).postln;
											wind = Window.new("SC->antescofo", Rect(10, 10, 820, 70)).front.view.keyDownAction = {|view, char, modifiers, unicode, keycode| if (keycode==53){wind.close}} ;
											ante_txt = TextView(wind, Rect(10, 10, 800, 50)).background_(Color.grey);

											ante_txt.string = "crea_track8 track"++100.rand++" 0 0 0".scatArgs(*[dico_keys.asArray, dico_antes.atAll(modules).asArray.postcs].flop.flatten(3).drop(1));
											ante_txt.select(0, ante_txt.string.size); //selecciona texto
											ante_txt.setStringColor(Color.white, 0, ante_txt.string.size)
		...
		//scroll.front;
		//auxViewSroll.front;
		win.front.view.keyDownAction = {|view, char, modifiers, unicode, keycode| if (keycode==53){win.close}};
	...
	makeEntry {|incr, name, background, synthtype, model| // Crea nuevo modulo
		//name.postln;
		//container.add(incr ->
					...
					HLayout(
						...
						but_win_mod = Button().maxHeight_(19).fixedWidth_(100).states_([[name, Color.green(0.6), Color.gray(0.3)]]).action_({ //|val|
							...
							if (modwin[index1].isNil) { // si la ventana esta cerrada
								...
								multislidparams = listmodules[index1][3].deepCopy;
								multislidparams.postln;
								...
								gui = TModuleGUI2(ttrack, name.asClass, synthe[index1], slidparams, multislidparams,  modgui[index1], model, others); //store[index1][2]
	...
	addmodule {|module, xfade, par, multi, otros|
		var temp, addsynth, main, background, sliderspec, multisliderspec, typesynth, multiparalist = List[], model_params = ();
		// module.asClass.def;
		main = module.asClass.main;
		background = module.asClass.background;
		typesynth = module.asClass.synthtype;
		...
		module.asClass.paramslider.collect({|parameter|
			model_params.put(parameter.key, parameter.value.default)
		});
		...
		sliderspec = module.asClass.paramslider.collect({|x| x.value});
		// multisliderspec = module.asClass.parammultislider.collect({|x| x.value});
		multisliderspec = module.asClass.parammultislider;

		sliderspec.collect({|sp, i|
			params[i] = sp.unmap(params[i])
		});

		/*		if (multisliderspec.notNil) {
		multisliderspec.collect({|sp, i|
		"_________________".postln;
		sp.postln;
		paramsmulti.postln;
		// paramsmulti[i] = sp.unmap(params[i])
		"_________________".postln;
		});
		};*/

		if (multisliderspec.notNil) {
			// "mutilsider_parammmmmmmmmmmmmssssss_formato1".postln;
			if(multi.notNil){
				// {0.01.wait;{
				paramsmulti = multi;
				// "mutilsider_parammmmmmmmmmmmmssssss_formato2".postln;
				paramsmulti.postln;
				// "mutilsider_parammmmmmmmmmmmmssssss_formato3".postln;
				// }.defer}.fork;
				/*				multisliderspec.collect({|sp, i|
				paramsmulti[i] = sp.unmap(paramsmulti[i])*/
				/*				"mutilsider_parammmmmmmmmmmmmssssss_formato1".postln;
				multi.postln;
				multisliderspec.postln;
				"mutilsider_parammmmmmmmmmmmmssssss_formato2".postln;
				multisliderspec.collect({|sp, i|
				var multislider_name, multislider_sp;
				multislider_name = sp.key;
				multislider_name.postln;
				multislider_sp = sp.value[0];
				multislider_sp.postln;
				// paramsmulti[i] = multislider_sp.unmap(multi[i]);
				/*multislider_sp = sp[multislider_name];
				multislider_sp.postln;*/
				// multi.postln;
				// paramsmulti[i] = sp[1].unmap(multi[i]);
				});
				"mutilsider_parammmmmmmmmmmmmssssss_formato3".postln;*/
				// });
				// multiparalist = multi;
			}{
				multisliderspec.collect({|sp, i| //recupera valores de las synth activas desde Antescofo
					var multislider_name, multislider_sp, multi_size;
					// "mutilsider_parammmmmmmmmmmmmssssss_formato4".postln;
					multislider_name = sp.key;
					multislider_sp = sp.value[0];
					multi_size = sp.value[1];
					multislider_name.postln;
					addsynth.getn(multislider_name, multi_size , {|val| //recupera valores por defecto
						// multiparalist.add(multislider_sp.unmap(val));
						multiparalist.add(val);

					});
				});
				paramsmulti = multiparalist;
			};
		};

		// {0.01.wait;{ // para esperar y sincronizar la respuesta del servidor de los multisliders default

		/*			"mutilsider_parammmmmmmmmmmmmssssss_formato5".postln;
		paramsmulti.postln;
		multi.postln;
		"mutilsider_parammmmmmmmmmmmmssssss_formato5".postln;*/

		if (typesynth == \in) {
			temp = this.makeEntryIn(inc, module, background, typesynth, model);
		}{
			temp = this.makeEntry(inc, module, background, typesynth, model);
		};
		/*"mutilsider_parammmmmmmmmmmmmssssss_formato2".postln;
		paramsmulti.postln;
		"mutilsider_parammmmmmmmmmmmmssssss_formato2".postln;*/
		...
		container = index.order.order;
		mvc_model.insert(indexof, model);
		...
		if(self[\sl][key].notNil) {
		self[\sl][key].value = value;
		};
		});*/


		inc = inc + 1;
		/*			"mutilsider_parammmmmmmmmmmmmssssss_formato6".postln;
		store.postln;
		"mutilsider_parammmmmmmmmmmmmssssss_formato6".postln;
		// }.defer}.fork;*/
	...
	addmodfromsynth {|module, synth|
		...
		var parados, parados2, temp, addsynth, main, background, sliderspec, multisliderspec, sliderskey, multisliderskey, typesynth, xfade, par, multi, otros, model_params = ();
		var temparams, tempmultiparams, amppar, ampparspec, mainpar, mainparspec, paralist = List[], multiparalist = List[];
		// module.asClass.def;
		[module, synth].postln;
		...
		multisliderspec = module.asClass.parammultislider;
		// multisliderskey = module.asClass.parammultislider.collect({|x| x.key});
		/*		"mutilsider_parammmmmmmmmmmmmssssss".postln;
		multisliderspec.postln;
		multisliderskey.postln;
		"mutilsider_parammmmmmmmmmmmmssssss".postln;*/
		// params = sliderskey;
		/*"parte3-22".postln;
		sliderskey.postln;
		...
		multisliderspec.collect({|sp, i| //recupera valores de las synth activas desde Antescofo
			...
			multi_size = sp.value[1];
			multislider_name.postln;
			synth.getn(multislider_name, multi_size , {|val|
				// multiparalist.add(multislider_sp.unmap(val));
				multiparalist.add(val);
		...
		{0.01.wait;{ // para sincronizar la respuesta del servidor
	...
	addmodfromantes {|module, synth|

		var parados, parados2, temp, addsynth, main, background, sliderspec, multisliderspec, sliderskey, multisliderskey, typesynth, xfade, par, multi, otros, model_params = (), node_id;
		var temparams, tempmultiparams, amppar, ampparspec, mainpar, mainparspec, paralist = List[], multiparalist = List[];
		// module.asClass.def;
		[module, synth].postln;
		// params = Array.new; //fill(module.asClass.paramslider.size,0);
		...
		{0.01.wait;{ // para sincronizar la respuesta del servidor
			...
			mvc_model_dico.add(node_id -> model);
			"mvc_model".postln;
			mvc_model_dico.postln;
	...
	modalGui { |presetreplace| // store module, crossfade, params, multiparams, buffers (no implementado)
		var x, a, presetname, slidparams = List[], multiparams = List[], store2, store3, store4, fadestore, spec1 = List[], modules, sliderspec, multisliderspec;
		...
		/*store2 = Array.fill(listmodules.size, List[]);
		store3 = Array.fill(listmodules.size, List[]);
		store2.postln;
		store3.postln;
		// slidparams = listmodules.collect(_[2]).deepCopy;

		modules = listmodules.collect(_[0]).deepCopy;
		listmodules.postln;
		// slidmultiparams = listmodules.collect(_[3]).deepCopy;
		slidparams.postln;
		slidmultiparams.postln;
		...
		// "----------------------STORE".postln;
		//
		// {0.2.wait;{	store.postln;
		// 	"----------------------STORE_presets".postln;
		// }.defer}.fork;
		// "----------------------STORE".postln;
