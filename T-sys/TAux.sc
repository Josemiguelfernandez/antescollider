...
TAux {
	...
	var <win, scroll, inc, canvas, layout, i = 0, insertMod, makeEntry, view, viewindex, children, index, indexof, viewobj, container, synthe, solo, outsynth, insynth, inbus, <>bus, <>group, levelmeter, slider, ampval, xfadeval, mainparam, mainparamdefault = 0.1, dbspec, textoutputlev, <>numoutputbus, <>numInAuxbus, <>listmodules, <>store_preset, menuitems, <>menu, <>tmute, <>tsolo, params, paramsmulti, <>specs, multispec, store, <modwin, ampknob, mainknob, xfadeknob, modgui, <>rand, others, buffer, <>ttrack, <>but_win_mod, <>slidparamsext;

	*new {|track|
		^super.newCopyArgs(track).initTTracks(track);
	}
	...
	initTTracks {|track|
		...
		win = Window.new(ttrack, Rect(Window.screenBounds.width-300,0,310,450)).background_(Color.new255(120, 120, 120)).alpha_(0.9); // false
		...
		group = Group.new(addAction: 'addToTail');
		bus = Bus.audio(Server.default, 8);
		//inbus = Bus.audio(Server.default, 1);

		//insynth = AudioInAux.synthInAux(group, bus, bus.index);

		outsynth = AudioOut8.synth(group, bus, bus.index);
		...
		win.layout = VLayout(
			scroll = ScrollView().maxHeight_(320).maxWidth_(310),
				...
				VLayout(
					View().maxWidth_(310).layout_(
						HLayout(
							...
							VLayout(
								numInAuxbus = NumberBox().maxHeight_(20).fixedWidth_(25).value_(0).action_({|val|
									insynth.set(\in, val.value)}),
								numoutputbus = NumberBox().maxHeight_(20).fixedWidth_(25).value_(0).action_({|val|
									outsynth.set(\out, val.value)})
			...
			HLayout(
				menu = PopUpMenu().maxHeight_(20).maxWidth_(200).items_(menuitems).action_({|menu|
					...
					preset = Object.readArchive("./fxPresets/Tracks/" ++ menu.item);
					preset.collect({|mod|
						/*var tempmod;
						mod.postln;
				...
				.action_({
					...
					alert.actions = [ {"cancel".postln },
						{
							it = menu.item;
							("rm ./fxPresets/Tracks/"++ it).unixCmd;
			...
			Button().maxHeight_(20).maxWidth_(60).states_([["Load"]]).action_({
				...
				this.modclear(ttrack);
				test = Object.readArchive("fxPresets/Tracks/test2");
				test = test[\toto];
				test.collect({|mod|
					var tempmod;
					tempmod = mod[0].postln;
	...
	addmodule {|module, xfade, par, multi, otros|
		...
		if ((typesynth == \in)|| (typesynth == \gen)) {
			"Only treatments!!".postln;
		}{
			temp = this.makeEntry(inc, module, background, typesynth);
		};
		...
		*/
		...
		if (presetreplace.isNil) {
			...
			.action_({ x.close ;
				...
				store.writeArchive("./fxPresets/Tracks/" ++ a.string);
			...
			store.writeArchive("./fxPresets/Tracks/" ++ menu.item);
		}
	...
	menucollect {
		...
		var myPath;
		myPath = PathName.new("./fxPresets/Tracks");
