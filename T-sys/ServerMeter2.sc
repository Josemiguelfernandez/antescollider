ServerMeterView2 {
	...
	*getWidth { arg numIns, numOuts, server;
	...
	init { arg aserver, parent, leftUp, anumIns, anumOuts;
		...
		/*a = { Button() } ! 10;
		parent.layout = HLayout(*a);*/

		// parent.layout_(HLayout(
		//
		// 	// *outmeters
		...
		if(numOuts > 0) {
			...
			// subviews = (outmeters[0..(numOuts-1)]);


			// View(parent).layout_(HLayout(
			parent.layout_(HLayout(
...
ServerMeter2 {
	...
	*new { |server, numIns, numOuts|
		...
		window = Window.new(server.name ++ " levels (dBFS)",
			Rect(5, 305, ServerMeterView2.getWidth(numIns, numOuts), ServerMeterView.height),
			false);
