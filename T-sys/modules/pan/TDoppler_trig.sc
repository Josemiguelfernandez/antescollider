TDoppler_trig : Tmodule2 {

	*def
	{


	^SynthDef(\TDoppler_trig, {|in = 0, inbus, ingain = 0, out = 0, outbus, outgain = 0, xFade = 1, speed = 1, dist = 10, amp = 0, panoffset = 0, dir = 1, matrix_ramp = 0.01, gate = 1, free = 1, t_trig = -1, run = 0|
			...
			// y = LFSaw.kr(speed, 0, 100).poll;
			y = (Sweep.ar(t_trig, 1/speed*run)).linlin(0, 1, -100, 100, \minmax);
	...
	*metadata
	{
		^(metadata: (
			...
			sliders: [
				...
				\run -> ControlSpec(0, 1,\lin,1, 0,\run),
