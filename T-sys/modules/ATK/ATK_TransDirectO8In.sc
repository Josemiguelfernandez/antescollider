ATK_TransDirectO8In : Tmodule2 {

	*def
	{
		^SynthDef(\aTK_TransDirectO8In, {|in = 0, inbus, ingain = 0, out = 0, outbus, outgain = 0, xFade = 1, angle = 1.5, dist = 3, azim = 0, amp = 0, matrix_ramp = 0.01, gate = 1, free = 1|
...
/*			encoderMatrix = [
			...
			// ------------------------------------------------------------
			// transform
			sig = FoaTransform.ar(sig, 'directO', angle);
	...
	*metadata
	{
		^(metadata: (
			synthdefname: "aTK_TransDirectO8In",
			type: \fx,
			main: \angle,
			sliders: [
