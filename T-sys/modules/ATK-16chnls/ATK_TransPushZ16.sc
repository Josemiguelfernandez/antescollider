ATK_TransPushZ16 : Tmodule2 {

	*def
	{
		^SynthDef(\aTK_TransPushZ16, {|in = 0, inbus, ingain = 0, out = 0, outbus, outgain = 0, xFade = 1, angle = 1.5, dist = 3, azim = 0, elev = 0, angle2 = 0, amp = 0, matrix_ramp = 0.01, gate = 1, free = 1|
...
/*			encoderMatrix = [
			...
			sig = FoaTransform.ar(sig, 'rtt' , (azim+22.5)*(pi/180), (elev+22.5)*(pi/180), (angle2+22.5)*(pi/180) ); //degree to radians
			sig = FoaTransform.ar(sig, 'pushZ', angle); //, (azim+22.5)*(pi/180), (elev+22.5)*(pi/180));
	...
	*metadata
	{
		^(metadata: (
			...
			sliders: [
				...
				\angle2 -> ControlSpec(0, 360,\lin, 0.001,0,\angle2),
