TGrainEnv_8in_ext : Tmodule2 {

	*def
	{


		^SynthDef(\tGrainEnv_8in_ext, {|in = 0, inbus, ingain = 0, out = 0, outbus, outgain = -120, xFade = 1, buf = 1, amp = 0, trate =  60, rate = 1, rateDev = 0, tratedev = 0, pos = 0, panDev = 0, dur = 0.1, matrix_ramp = 0.01, startPos = 0.1, gate = 1, free = 1, t_trig = 0, centerpos = 0, bufenv = -1|
			...
			sig = GrainBuf.ar(8, t_trig, dur, buf, rate, centerpos, 2, pos, bufenv);
	...
	*metadata
	{
		^(metadata: (
			synthdefname: "tGrainEnv_8in_ext",
			type: \fx,
			main: \amp,
			sliders: [
