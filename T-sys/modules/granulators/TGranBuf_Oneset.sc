TGranBuf_Oneset : Tmodule2 {


	*def
	{
		^SynthDef(\granEnv_8_one_onset, { |in = 0, out = 0, inbus, ingain = 0, outbus = -2, trigRate = 1, trigRateDev = 0, buf = 132, envbuf = -1, rate = 1.0, xFade = 1, thresh = 0.5, rateDev = 0, startPos= 0, centerDev = 0, dur = 0.25, durDev = 0, pan = 0, panDev = 0, amp=0, ampDev = 0, reset = 0, interpol = 2, matrix_ramp = 0.01, gate = 1, free = 1|

			var sig, impulso, posDevia, pos, trigRate2, envgate, envpause, chain, onsets, trig, granout, cross;
	...
	*metadata
	{
		^(metadata: (
			synthdefname: "granEnv_8_one_onset",
			type: \fx,
			main: \trigRate,
			sliders: [
				\thresh -> ControlSpec(0, 1, \lin, 0.001, 0.5, \thresh),
				\reset -> ControlSpec(0, 1, \lin, 1, 0, \reset),
