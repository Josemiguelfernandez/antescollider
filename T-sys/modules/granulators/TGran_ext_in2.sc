TGran_ext_in2 : Tmodule2 {


	*def
	{
		^SynthDef(\granEnv_ext_in2, { |in = 0, inbus, ingain = 0, outgain = 0 out = 0, outbus = -2, t_trig = 0, trigRate = 1, trigRateDev = 0, buf, envbuf = -1, buf_trig = 1, rate = 1.0, rateDev = 0, startPos= 0.5, centerDev = 0, dur = 0.2, durDev = 0, pan = 0, panDev = 0, amp= -10, ampDev = 0, interpol = 2, matrix_ramp = 0.01, gate = 1, free = 1|
			...
			pos = posDevia + (startPos / BufDur.kr(buf)); //TGran_ext_in2 usa BufDur para calcular posicion
			...
			sig = GrainBuf.ar(8, t_trig, dur, buf, rate, pos, interpol, pan ,envbuf);
